...
def select_class():
    if request.method == "POST":
        ...
        if number_student < 1 or match is None:
            return render_template('final.html', selected_class=None, detection=detection, url=url )
        ...
        for i in range(0, number_student):
            ...
            if match == studentInfo["name"]:
                ...
                if selected_class in studentInfo["classes"]:
                    ...
                    return render_template('final.html', selected_class=selected_class, detection=detection, url=url )
                else:
...
def gen_frames():
    ...
    while True:
        ...
        if not success:
            ...
            frame = imutils.resize(frame, width=int(frame.shape[1] * 0.5), height=int(frame.shape[0] * 0.5))
